      ...
      <v-flex xs12 sm8 md8>
        ...
        <v-card class="elevation-12"> 
          ...
          <v-card-title primary-title>
            <h3 class="headline mb-0">Edit a new remark for Inquiry ID: {{ inquiry.id }}
...
<script type="text/javascript">
  ...
  export default {
    ...
    mounted() {
      ...
      this.form.get(`/api/inquiries/${this.$route.params.inquiry_id}/remarks/${this.$route.params.remark_id}`)
        .then(data => {
    ...
    methods: { 
      update() {
        this.form.patch(`/api/inquiries/${this.$route.params.inquiry_id}/remarks/${this.$route.params.remark_id}`)
          .then(data  =>  {
            this.$router.push(`/inquiries/${this.inquiry.id}`);
          })
          .catch(errors => {
